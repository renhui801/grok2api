...
func TestActiveTeamModelRateLimitUnknownTeamIDSharesObservedTeam(t *testing.T) {
	now := time.Now().UTC()
	const model = "grok-imagine-image-quality"
	const teamID = "00000000-0000-0000-0000-000000000014"
	first := account.Credential{ID: 11, Provider: account.ProviderConsole}
	second := account.Credential{ID: 12, Provider: account.ProviderConsole}
	service := NewService(nil, nil, nil, nil, nil, nil, nil, 3)
	service.markTeamModelRateLimit(first, model, provider.RateLimitMetadata{
		Scope: provider.RateLimitScopeRPS, TeamID: teamID, Model: model, Actual: 2, Limit: 2, RetryAfter: 2 * time.Second,
	}, now)
	limited, ok := service.activeTeamModelRateLimit(second, model, now)
	if !ok {
		t.Fatal("empty TeamID accounts must share the Team observed in a 429 body")
	}
	if limited.TeamFingerprint != shortTeamFingerprint(rateLimitTeamFingerprint(teamID)) {
		t.Fatalf("fingerprint = %q", limited.TeamFingerprint)
	}
	otherModel := account.Credential{ID: 13, Provider: account.ProviderConsole}
	if _, blocked := service.activeTeamModelRateLimit(otherModel, "grok-other", now); blocked {
		t.Fatal("unknown-team binding must stay scoped to the limited model")
	}
}

func TestGenerateImageEmptyTeamIDWaitsInsteadOfSpraying(t *testing.T) {
	ctx := context.Background()
	database, err := relational.OpenSQLite(ctx, filepath.Join(t.TempDir(), "image-empty-team-rate-limit.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer database.Close()
	...
	keyRepo := relational.NewClientKeyRepository(database)
	credentials := make([]account.Credential, 0, 3)
	for index, name := range []string{"empty-team-a", "empty-team-b", "empty-team-c"} {
		credential, _, createErr := accountRepo.UpsertByIdentity(ctx, account.Credential{
	...
	key, err := keyRepo.Create(ctx, clientkey.Key{
		Name: "image-empty-team-key", Prefix: "imageempty", SecretHash: strings.Repeat("b", 64), EncryptedSecret: "encrypted-key",
		Enabled: true, RPMLimit: 60, MaxConcurrent: 4,
	})
	if err != nil {
		t.Fatal(err)
	}
	adapter := &emptyTeamImageRateLimitAdapter{retryAfter: 40 * time.Millisecond}
	registry := provider.NewRegistry(adapter)
	sticky := memory.NewStickyStore()
	accountService := accountapp.NewService(accountRepo, auditRepo, memory.NewDeviceSessionStore(), sticky, registry, testCipher(t), nil)
	selector := NewSelector(accountRepo, memory.NewConcurrencyLimiter(), sticky, registry, time.Hour, time.Second, time.Minute)
	...
	result, err := service.GenerateImage(ctx, ImageGenerationInput{
		RequestID: "req-image-empty-team", ClientKey: key, PublicModel: model,
		Prompt: "test", Count: 1, ResponseFormat: "url",
	})
	if err != nil || result == nil || result.StatusCode != http.StatusOK {
		t.Fatalf("result = %#v, err = %v", result, err)
	...
	if len(attempts) != 2 {
		t.Fatalf("empty TeamID 429 must wait and retry instead of spraying the pool, attempts=%#v", attempts)
	}
	if attempts[0] != attempts[1] {
		t.Fatalf("empty TeamID 429 must retry the same account after wait, attempts=%#v", attempts)
	}
}
...
type emptyTeamImageRateLimitAdapter struct {
	mu           sync.Mutex
	attempts     []uint64
...
func (a *emptyTeamImageRateLimitAdapter) Provider() account.Provider {
	return account.ProviderConsole
}
func (a *emptyTeamImageRateLimitAdapter) Definition() provider.Definition {
	definition := testConversationDefinition(account.ProviderConsole)
	definition.Media.ImageGeneration = true
	return definition
}
func (a *emptyTeamImageRateLimitAdapter) GenerateImage(_ context.Context, request provider.ImageGenerationRequest) (*provider.Response, error) {
	a.mu.Lock()
	a.attempts = append(a.attempts, request.Credential.ID)
	now := time.Now()
	if a.blockedUntil.IsZero() {
		a.blockedUntil = now.Add(a.retryAfter / 2)
	}
	blocked := now.Before(a.blockedUntil)
	...
	return &provider.Response{
		StatusCode: http.StatusTooManyRequests, Status: "429 Too Many Requests",
		Header: http.Header{"Content-Type": {"application/json"}, "X-Should-Retry": {"false"}},
		Body:   io.NopCloser(strings.NewReader(`{"code":"resource-exhausted","error":"Too many requests for team 00000000-0000-0000-0000-000000000014 and model grok-imagine-image-quality. Requests per Second (actual/limit): 2/2."}`)),
		RateLimit: &provider.RateLimitMetadata{
			Scope: provider.RateLimitScopeRPS, TeamID: "00000000-0000-0000-0000-000000000014", Model: request.Model,
			Actual: 2, Limit: 2, RetryAfter: retryAfter,
		},
	}, nil
}
func (a *emptyTeamImageRateLimitAdapter) Attempts() []uint64 {
	a.mu.Lock()
	defer a.mu.Unlock()
	return append([]uint64(nil), a.attempts...)
}
